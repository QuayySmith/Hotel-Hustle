...
export default function LandscapeOnly({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex flex-col items-center justify-center bg-radial from-slate-900 via-slate-950 to-black p-6 text-center select-none">
      ...
      {/* Warning Box */}
      <div className="relative z-10 max-w-sm bg-slate-900/80 border-2 border-fuchsia-500/50 rounded-3xl p-8 shadow-2xl backdrop-blur-md animate-bounce-subtle flex flex-col items-center">
        {/* Glow Bar */}
        ...
        <div className="flex flex-col gap-3.5 w-full">
          <button
            ...
            className="w-full py-3 bg-gradient-to-r from-cyan-500 to-fuchsia-600 hover:scale-105 active:scale-95 text-white font-sans text-xs font-black tracking-widest rounded-2xl shadow-lg transition-all cursor-pointer"
          >
            PLAY IN PORTRAIT ANYWAY
          </button>

          <div className="inline-flex items-center justify-center gap-2 px-4 py-2 bg-slate-950 border border-slate-800 rounded-xl">
            <span className="w-2.5 h-2.5 bg-emerald-500 rounded-full animate-ping"></span>
            <span className="font-mono text-[10px] text-slate-300 font-medium">OPTIMIZED FOR LANDSCAPE</span>
          </div>
        </div>
      </div>
    </div>

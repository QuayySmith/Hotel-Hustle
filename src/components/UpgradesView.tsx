...
import { ArrowLeft, DollarSign, ArrowUpRight, Check, Sparkles, Building } from 'lucide-react';
...
export default function UpgradesView({
  ...
  return (
    <div className="w-full h-screen bg-slate-950 text-white flex flex-col justify-between overflow-hidden select-none font-sans relative">
      ...
      <div className="absolute top-1/4 left-1/4 w-[30rem] h-[30rem] rounded-full bg-purple-600/10 blur-[100px] pointer-events-none"></div>
      ...
      <header className="relative z-10 w-full bg-slate-900/95 border-b border-slate-800 px-6 py-3 flex items-center justify-between shadow-md">
        <div className="flex items-center gap-3">
          <button 
            ...
            className="w-9 h-9 flex items-center justify-center rounded-xl bg-slate-800 border border-slate-700/60 hover:border-purple-500 hover:text-purple-400 transition-all cursor-pointer"
          >
            <ArrowLeft className="w-5 h-5" />
          </button>
          <div>
            <h1 className="font-sans text-base font-black tracking-wider text-purple-400 uppercase flex items-center gap-1.5">
              👑 SUITE UPGRADES SHOP
            </h1>
            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">
              Optimize rooms, kitchens, and lobby chandeliers for premium rent
            </p>
          </div>
        </div>
        ...
        <div className="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-4 pb-6">
          {upgrades.map((up) => {
            ...
            return (
              <div 
                key={up.id}
                className="bg-slate-950 border border-slate-850 rounded-3xl p-5 flex flex-col justify-between hover:border-purple-500/50 transition-all shadow-lg group relative overflow-hidden"
              >
                ...
                <div className="flex items-start gap-4 mb-4">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      {/* Level Badge */}
                      <span className="px-2 py-0.5 bg-purple-500/10 border border-purple-500/20 text-purple-300 rounded-md font-mono text-[9px] font-bold">
                ...
                <div className="flex justify-between items-center pt-3 border-t border-slate-900">
                  {/* Current perk level multiplier label */}
                  <div className="text-left">
                    <span className="text-[9px] text-slate-500 font-bold uppercase tracking-wider block">CURRENT BONUS</span>
                    <span className="text-xs font-bold text-purple-400 font-mono">
                      +{Math.floor((up.level * 15))}% yield
                    </span>
                  </div>
                  ...
                  {isMaxed ? (
                    ...
                    <button
                      ...
                      className={`flex items-center gap-1.5 px-4 py-2.5 rounded-xl font-sans font-black text-xs tracking-wide transition-all ${
                        canAfford 
                          ? 'bg-gradient-to-r from-purple-500 to-indigo-600 hover:scale-[1.03] text-white shadow-lg cursor-pointer' 
                          : 'bg-slate-900 border border-slate-850 text-slate-500 cursor-not-allowed'
                      }`}
                    >
                      <ArrowUpRight className="w-4 h-4" />
                      UPGRADE • ${up.cost.toLocaleString()}
                    </button>
                  )}
                </div>
              </div>
